...
void groupByKey(IntermediateInput *input, Output *output, int *result_count) {
  ...
  Output *out = (output + *result_count);
  out->doubled_value = input->doubled_value;
  out->line_numbers[0] = input->line_number;
  out->count = 1;
  *result_count += 1;
}

void reduce(Output *output) {
  printf("(%i, [", output->doubled_value);
  for (int i = 0; i < output->count - 1; i++) {
    printf("%i, ", output->line_numbers[i]);
  }
